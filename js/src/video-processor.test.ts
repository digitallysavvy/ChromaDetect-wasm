...
describe('VideoProcessor', () => {
  ...
  describe('calculateFrameTimestamps', () => {
    it('should calculate uniform timestamps', () => {
      const timestamps = (processor as any).calculateFrameTimestamps(10, 4, 'uniform', 30);
      expect(timestamps).toEqual([2, 4, 6, 8]);
    });

    it('should respect maxDuration', () => {
        const timestamps = (processor as any).calculateFrameTimestamps(100, 4, 'uniform', 10);
        expect(timestamps).toEqual([2, 4, 6, 8]);
    });
    
    it('should calculate keyframe strategy timestamps', () => {
        const timestamps = (processor as any).calculateFrameTimestamps(10, 10, 'keyframes', 30);
  ...
  it('should detect from HTMLVideoElement', async () => {
    ...
    // @ts-ignore
    (ChromaDetect as any).mock.instances[0].get_video_consensus = mockGetConsensus;
    ...
    const result = await processor.detectFromVideo(video, { frameSampleCount: 2 });
  ...
  it('should handle File input and load video', async () => {
    ...
    vi.spyOn(document, 'createElement').mockImplementation((tagName) => {
        ...
        if (tagName === 'canvas') {
             const c = { 
                 width: 0, height: 0,
                 getContext: () => ({ drawImage: vi.fn(), getImageData: () => new ImageData(10,10) }) 
             } as any;
             return c;
        }
        return {} as any;
    ...
    Object.defineProperty(videoMock, 'currentTime', {
        ...
        set: () => {
             setTimeout(() => {
                 const call = (videoMock.addEventListener as any).mock.calls.find((c: any) => c[0] === 'seeked');
                 if (call) call[1]();
             }, 10);
        }
    });
  ...
  it('should handle video load error', async () => {
    ...
    await expect(processor.detectFromVideo(file)).rejects.toThrow('Failed to load video');
  ...
  it('should handle frame extraction timeout', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to extract frame'), expect.any(Error));
    warnSpy.mockRestore();
